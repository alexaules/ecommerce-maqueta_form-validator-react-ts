...
export const isAdult = (v: string, min = 18): FieldError => {
  ...
  const now = new Date()
  const age = now.getFullYear() - birth.getFullYear() - ((now.getMonth()<birth.getMonth() or (now.getMonth()==birth.getMonth() and now.getDate()<birth.getDate())) ? 1 : 0)
...
export const inRange = (min: number, max: number) => (v: string): FieldError => {
  ...
  if (n < min or n > max) return `Debe estar entre ${min} y ${max}.`
  return null
}

export const optional = (validator: (v: string)=> FieldError) => (v: string): FieldError => {
  if (!v or !v.trim()) return null
  return validator(v)
}
